...
export const TaskItem: React.FC<TaskItemProps> = ({ task }) => {
  const { handleDeleteTask, handleUpdateTaskStatus, handleUpdateTask } =
    useTasksProvider();
  ...
  return (
    <div className="TaskItem" data-testid="TaskItem">
      <div className="TaskOverview" data-testid="TaskOverview">
        <div>
          <label
            className={"StatusLabel " + task.status}
            onClick={() => handleUpdateTaskStatus(task.ID)}
          >
            {OVERVIEW_ITEM_TITLES[task.status]}{" "}
          </label>
      ...
      >
        ...
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            handleUpdateTask(task, title, description, deadline);
        ...
        >
          ...
          <input
            ...
            onBlur={({ target: { value } }) =>
              setDeadline(dateFormatter(value.replace(/[^0-9/]+/g, "")))
            }
          />
